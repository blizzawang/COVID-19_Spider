...
class CoronaVirusSpider():
    ...
    def coll_statistics_corona_virus_data_of_province(self):
        ...
        corona_virus_list_province = self.parse_corona_virus_data(last_day_corona_virus_data_of_province,
                                                                  '爬取1月20日以来全国各省的疫情信息')
        # 保存数据
        self.save_file(corona_virus_list_province, 'data/corona_virus_list_province.json')

    def coll_new_day_data_of_province(self):
        """
        爬取全国各省最新一日的疫情数据
        :return:
        """
        # 加载最近一日全国各省的疫情数据
        last_day_corona_virus_data_of_province = self.load_file('data/coll_last_day_corona_virus_data_of_province.json')
        # 定义一个字典，存放省份与其对应的统计数据url
        province_statisticsData_url = {}
        deal_data_list = []
        # 遍历各省数据
        for province in last_day_corona_virus_data_of_province:
            province_statisticsData_url[province['provinceShortName']] = province['statisticsData']
        # 遍历字典
        for province_name in tqdm(province_statisticsData_url, '爬取数据中'):
            # 获取到省份对应的统计数据url
            url = province_statisticsData_url[province_name]
            # 获取疫情的统计数据
            statistics_data = self.send_request(url)
            # 将其转为Python数据
            py_data = json.loads(statistics_data)['data']
            # 获取列表的最后一个字典，即为最新的疫情数据
            py_data = py_data[-1]
            # 在字典中添加国家名称
            py_data['provinceShortName'] = province_name
            # 将其加入列表
            deal_data_list.append(py_data)
        print(deal_data_list)
        # 将数据保存到文件
        self.save_file(deal_data_list, 'data/coll_new_day_data_of_province.json')
    ...
    def run(self):
        ...
        self.coll_new_day_data_of_province()
